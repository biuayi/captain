...
| 0008 | `event_whitelist_entry` | + `company text`(可空,多企业消歧)、+ `claimed_jwt_jti text`(防一号多端)；**保留** `phone_last4 NOT NULL`+`^[0-9]{4}$`(必填辅助因子)；**放宽** `phone_number` DROP NOT NULL、`name` 删 `ewe_name_nonempty` 改可空(D2 可选)；唯一键 `uniq_ewe_event_employee` **替换为** `UNIQUE(event_id, company_norm, employee_number)` |
| 0008 | `event` | + `timezone text NOT NULL DEFAULT 'Asia/Shanghai'`（D4 活动方设定，驱动 R1 日界）、`strict_fingerprint bool NOT NULL DEFAULT false`（D3）、`identity_require_name/identity_require_phone/identity_multi_company bool NOT NULL DEFAULT false`（D2 身份因子开关） |
...
**功能**
1. 活动方导入员工名单（CSV）。**D2 字段集（用户已定）**：
   - 员工工号 `employee_number` —— **必填**（业务主键）。
   - 手机号后4位 `phone_last4` —— **必填**，登录强制辅助校验因子（沿用 0003 既有 `NOT NULL`+`^[0-9]{4}$`）。
   - 员工姓名 `name` —— **可选**（默认仅展示；活动方可勾 `identity_require_name` 作登录校验）。
   - 员工手机号全号 `phone_number` —— **可选**（企业合规敏感；活动方勾 `identity_require_phone` 才作更强校验并需名单含全号）。
...
| `POST /p/e/{event_id}/login` | **新增** `{employee_number,phone_last4,name?,phone?,company?,fingerprint,turnstile_token}`（工号+后4位必填；name/phone/company 按活动开关必填）→ `{token, participant:{id,name}}`；错误码 `bad_credentials/missing_factor/account_locked/captcha_failed/event_inactive` |
...
**数据库**：迁移 0008——`event_whitelist_entry` + `company text`(可空)、+ `claimed_jwt_jti text`；**保留** `phone_last4 NOT NULL`+`^[0-9]{4}$` CHECK（必填辅助因子，沿用 0003）；**放宽** `phone_number` DROP NOT NULL（全号可选）、`name` 删 `ewe_name_nonempty` CHECK 改可空（姓名可选）；**替换唯一键** `uniq_ewe_event_employee` → `UNIQUE(event_id, company_norm, employee_number)`（`company_norm = lower(coalesce(nullif(btrim(company),''),''))`，单企业空串兼容）；`event` + `identity_require_name/identity_require_phone/identity_multi_company bool DEFAULT false`。复用 0003 指纹列。**注**：0008 含约束变更（非纯 additive），v1 演进期数据可重建则安全；生产有数据按迁移注释处理。
...
**决策与假设（D2 已定）**：① 必填=工号+手机后4位；可选=姓名/全手机号/企业名（活动方按开关启用为登录校验因子）；② "用户ID"=名单条目，事件内由 `company+employee_number` 唯一；③ 手机后4位为强制辅助校验（沿用 0003，无需名单含全号，照顾企业合规）；④ 多企业联办用 `company` 消歧并并入唯一键，单企业空串兼容；⑤ 活动方可解绑设备让用户换机重登（解绑保留旧记录、审计）。均按用户最新指示落定。
...
5. 名单导入（refactor）：可变表头 `employee_number,phone_last4[,name][,phone][,company]`（工号+后4位必填），导入时活动方勾选 `identity_require_name`/`identity_require_phone`/`identity_multi_company`（写入 `event`，校验名单列与勾选一致；提供全号则校验/派生后4位）；活动方可对名单条目执行**设备解绑**（详见 SS-2）。
...
**功能**
1. 活动方按活动看参与明细，字段**对齐原始需求**：记录ID(`participation.id`)、用户名(`whitelist.name`，可选)、工号(`employee_number`)、企业名(`whitelist.company`，多企业联办时有值)、手机后4位(`phone_last4`，必有)、手机全号(`phone_number`，若名单提供：脱敏显示/导出全号)、参与时间(首次/各步)、用户位置(`checkin_day`/`participation` geo)、**用户设备ID**(`participation.device_id`，导出可见)、**用户浏览器指纹**(`participant.fingerprint_hash`，导出可见)、**数据栏一**(`data_field_1` 文本)、**数据栏二**(`data_field_2` OSS 位置→签名链接)。受 `can_view_records` 控制；查看记 PII 审计。
...
`0006` 平台基座 → `0007` 模板 → `0008` 参与者登录(企业名/手机可选/唯一键替换)+多日签到+活动时区/严格指纹/告警表 → `0009` 考试题库 → `0010` 抽奖+多奖池(pool/membership/prize/rig/result) → `0011` 记录字段+环节漏斗(current_stage/completed_all)。绝大多数 `IF NOT EXISTS`/`ADD COLUMN IF NOT EXISTS`/`ON CONFLICT`。**例外**：0008 含约束变更（`event_whitelist_entry` 放宽 `phone_number`/`name` 可空、保留 `phone_last4` 必填、替换唯一索引为含 `company_norm`），均以 `ALTER ... DROP CONSTRAINT IF EXISTS` / `DROP INDEX IF EXISTS` + 重建实现幂等；演进期 v1 数据可重建故安全，生产有热数据须按 0002 注释思路（CONCURRENTLY 重建唯一索引、分步放宽约束）离线/灰度处理。
...
| D2 | **用户已定**：必填=工号+手机后4位+活动ID；可选=姓名/全手机号/企业名(活动方按开关启用为校验因子)；多企业加企业名并入唯一键；活动方可解绑用户设备(保留记录+审计) | 因子可配，单企业 company 空串兼容 | — |
